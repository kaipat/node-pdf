...
module.exports = class TablePDF extends PDFKit {
  constructor(options = {}) {
    ...
    this.headerConfig = {
      ...
      font: "Noto_Sans_SC", borderColor: "#FFFFFF",
      backgroundColor: options.headerConfig && options.headerConfig.backgroundColor
        ? options.headerConfig.backgroundColor
        : "#8B8787"
    };
    this.cellConfig = {
      fontSize: 9, font: "Noto_Sans_SC",
      padding: { top: 5, left: 5, right: 5, bottom: 5 },
      justify: "start", align: "center",
      color: "#3A3A3A",
      backgroundColor:
        options.cellConfig && options.cellConfig.backgroundColor && options.cellConfig.backgroundColor.length
          ? options.cellConfig.backgroundColor
          : ["#ffffff", "#D8D8D8"]
    };
    this.pageConfig = {
      backgroundColor: options.pageConfig ? options.pageConfig.backgroundColor : ""
    };
    this.registerFont(
      "Noto_Sans_SC",
      path.resolve(
        __dirname,
        "fonts/Noto_Sans_SC/NotoSansSC-Regular.otf"
      )
    );
    this.font("Noto_Sans_SC");
  }
   ...
   */
  pipeline(stream) {
    if (!stream) throw new Error("argument stream is required");
    this.pipe(stream);
    this.addRows();
    this.end();
  }
  ...
  drawPageBackground() {
    if (this.pageConfig.backgroundColor) {
      this
        ...
        .fillColor(this.pageConfig.backgroundColor)
        .fill()
        .restore();
    }
  }
