...
module.exports = class TablePDF extends PDFKit {
  constructor(options = {}) {
    ...
    this.header = { backgroundColor: "#ff5252", color: "#f8fafc", fontSize: 16 };
  ...
  getRowConfig(rowConfig, options = {}) {
    ...
    const borderWidth = this.border.width;
    const { padding, justify, align, color, backgroundColor, fontSize } = this.cellConfig;
  ...
  addRows(start = 0) {
    ...
    for (let rowIndex = 0; rowIndex < rowData.length; rowIndex++) {
      ...
      for (let cellIndex = 0; cellIndex < row.length; cellIndex++) {
        ...
        if (cell.isHeader) console.log(cell);
